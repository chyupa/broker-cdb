		...
		<table class="form-table">
		  <tbody>
			...
			<tr>
			  ...
			  <td><input type="checkbox" name="user_profile_public" id="user_profile_public" value="1" <?php checked( 1, get_the_author_meta( 'user_profile_public', $contactoptions->ID ) ); ?> /></td>
			</tr>
			<tr>
			  <th>
			...
			<tr>
			 <th>
			  ...
			  <td>
				 <ul class="cs-form-element column-input has-border" style="width:30%;">
...
function cs_error_msg( $error_msg ) {
	...
	foreach ($error_msg as $value) {
		if ( !empty( $value ) ) {
			$msg_string .= '<div class="error" style="color:#ffffff;"><i class="icon-info-circle"></i> ' . $value . '</div>';
		}
	}
	$msg_string .= '</div>';
	return $msg_string;
...
function cs_success_msg( $successs_msg ) {
	$msg_string = '<div style="background:#aebc49;" class="messagebox messagebox-v1 has-radius alert alert-info align-left no_border">';
		foreach ($successs_msg as $value) {
		if ( !empty( $value ) ) {
			$msg_string .= '<div class="error" style="color:#000;"><i class="icon-check"></i> ' . $value . '</div>';
		}
	}
	$msg_string .= '</div>';
	return $msg_string;
...
// Upload Custom File
//=====================================================================
function cs_custom_file_upload( $upload_data ) {
...
//=====================================================================
function get_ad_paypal_form( $uid, $post_id ,$cs_directory_pkg_names, $payment_date, $addType , $is_featured ) {
	...
	if( isset( $cs_packages_options ) && is_array( $cs_packages_options ) && count( $cs_packages_options ) > 0 ){
		...
		if ( isset ( $is_featured ) && $is_featured == 'yes' && $is_true == true ) {
			$cs_new_price	= $cs_package_price + $package_featured_ads;
		}
		...
		if ( $dir_pkg['package_duration'] == 'unlimited' ) {
			...
			//update_post_meta( $post_id, '_pakage_meta', $dir_pkg );
			//update_post_meta((int)$post_id, 'dir_pkg_expire_date', $expire_date);
		}
		...
		/*if ( isset ( $cs_upgrade_featured ) && $cs_upgrade_featured == 'yes' ) {
			$cs_totalPrice	= $package_featured_ads;
		}*/
		
		if(isset($dir_pkg['package_title']))
			$custom_fields .= '<li><label>Title: </label>'.$dir_pkg['package_title'].'</li>';
		if ( $dir_pkg['package_duration'] == 'unlimited' ){
			$custom_fields .= '<li><label>Duration: </label>UNLIMITED</li>';
		} else {
		 if(isset($dir_pkg['package_duration']))
			$custom_fields .= '<li><label>Duration: </label>'.$dir_pkg['package_duration'].' no of days.</li>';
		}
		
		if(isset($dir_pkg['package_price']))
			$custom_fields .= '<li><label>Package Price: </label>'. $cs_package_price.'</li>';
		if(isset($package_featured_ads))
			$custom_fields .= '<li><label>'.__('Featured Ad Price','directory').': </label>'.$package_featured_ads.'</li>';
		if(isset($dir_pkg['package_price']))
			$custom_fields .= '<li><label>Total Price: </label>'.$cs_totalPrice.'</li>';

		$custom_fields	.= '</ul>';
		
		$packageInfo	= cs_allow_special_char( $custom_fields );
	...
	if( $package_price > 0 || ( isset( $package_featured_ads ) && $package_featured_ads > 1 && $directory_featured == 'yes')){
	...
	//==Update Featured 
	//update_post_meta( $post_id, 'directory_featured', 'no' );
	
	$cs_paypal_email = $cs_theme_options['paypal_email'];
	$paypal_content_button .= '<form name="PayPalForm" id="direcotry-paypal-form" action="'.$paypal_url.'" method="post">  
								<input type="hidden" name="cmd" value="_xclick">  
								<input type="hidden" name="business" value="'.sanitize_email($cs_paypal_email).'">
								<input type="hidden"  name="amount" value="'.absint($cs_totalPrice).'">
								<input type="hidden" name="item_name" value="'.sanitize_text_field(get_the_title($post_id)).'"> 
								<input type="hidden" name="item_number" value="'.sanitize_text_field($post_id).'">  
								<input name="cancel_return" value="'.esc_url(get_permalink($post_id)).'" type="hidden">  
								<input type="hidden" name="no_note" value="1">  
								<input type="hidden" name="notify_url" value="'.sanitize_text_field($cs_theme_options['dir_pkg_paypal_ipn_url']).'">
								<input type="hidden" name="lc">
								<input type="hidden" name="rm" value="2">
								<input type="hidden" name="custom" value="'.sanitize_text_field($uid.'_'.$cs_directory_pkg_names.'_'.$featured).'">  
								<input type="hidden" name="return" value="'.esc_url(get_permalink($post_id)).'">  
								<span class="donate-btn btn"><input class="bgcolr" type="submit" value="Submit"> </span>
							</form> ';
        ...
        <div class="cs-package-upper">
            ...
            <div class="inner-sec">
                ...
                if( isset( $directory_feature_price ) && $directory_feature_price <> '' )
                    echo cs_allow_special_char($currency_sign.intval($directory_feature_price));
                else
                    echo cs_allow_special_char($currency_sign.intval($cs_theme_options['directory_featured_ad_price']));
                ?>
